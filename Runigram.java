...
public class Runigram {
	...
	public static Color[][] read(String fileName) {
		...
		for(int i = 0; i < numRows; i++) {
			for(int j = 0; j < numCols; j++) {
				int redColor = in.readInt();
				int greebColor = in.readInt();
				int blueColor = in.readInt();
				image[i][j] = new Color(redColor, greebColor, blueColor);
			}
		}
		return image;
	}
	...
	private static Color luminance(Color pixel) {
		int redColor = pixel.getRed();
		int greenColor = pixel.getGreen();
		int blueColor = pixel.getBlue();
		int lum = (int)(0.299 * redColor + 0.587 * greenColor + 0.114 * blueColor);
	...
	public static Color[][] scaled(Color[][] image, int width, int height) {
		...
		for(int i = 0; i < height; i++) {
			for(int j = 0; j < width; j++) {
				scaledImage[i][j] = image[(int)(i * ((double)originalHeight / (double)height))][(int)(j * ((double)originalWidth / (double)width))];
			}
		}
		return scaledImage;
	}
	...
	public static Color[][] blend(Color[][] image1, Color[][] image2, double alpha) {
		int imageRows = image1.length;
		int imageCols = image1[0].length;
		Color[][] blendedImage = new Color[imageRows][imageCols];
	...
	public static void morph(Color[][] source, Color[][] target, int n) {
		...
		for(int i = 0; i <= n; i++) {
			blendedImage = blend(source, target, (n - i) / n);
